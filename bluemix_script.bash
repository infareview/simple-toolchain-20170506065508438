...
#### Log file extension ####
        extention='.log'
echo $1 > fname_tmp.txt 2>/dev/null
if [ $? -ne 0 ]
then
...
#### Mapping count in the Workflow ####
        count="$(grep  -c MAPPINGNAME $1 2>/dev/null)"
#### Extracting Session Tag value in Session.xml ####
                sed -n '/<SESSION DESCRIPTION/,/<\/SESSION>/p' $1 > Session.xml
#### Extracting MAPPING value in Session.xml ####
                                sed -n '/<MAPPING DESCRIPTION/,/<\/MAPPING>/p' $1 > Mapping.xml
#### Extracting Workflow Name ####
        wf_name="$(grep 'WORKFLOW DESCRIPTION' $1 | cut -d'"' -f12)"
        echo "========================== $wf_name ========================== " > $log
if [ $count -eq 0 ]
then
...
for (( j=1 ; j <= $count  ; j++ ))
        ...
        #### Write Backward Compatible Session ####
                        wbc="$(grep 'Write Backward Compatible Session Log File' abc.txt |awk '{print $10}' |sed 's/[=">/]//g')"
        ...
        echo -e '\n-----------------------------------------------------------------------------------------------------------------------------------------------------------------------' >> $log
                ...
                #### Parameter File used in Workflow ####
                        param="$(grep 'Parameter Filename' $1 |  cut -d'"' -f4|sed 's/&#x5c;/\//g'|sort|uniq |sed '/^$/d')"
                                        echo -e "   PARAMETER FILE : $param \n" >> $log
                                        echo '   CONNECTIONS : ' >> $log
                #### Connection Values used in Workflow ####
                        connection="$(grep 'CONNECTIONREFERENCE' $1 | cut -d'"' -f12 | sort|uniq|sed '/^$/d')"
